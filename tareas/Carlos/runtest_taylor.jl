...
@testset "pruebas realizadas" begin
    ...
    @test exp(Taylor(0,[0]))==Taylor(0,[1.0]) #exponencial
    @test (Taylor(1,[2,0]))^2==Taylor(1,[4,0]) #potencia
    @test sin(Taylor(1,[0,0]))==Taylor(1,[0,1])#seno
    @test cos(Taylor(1,[0,0]))==Taylor(1,[1,0])#coseno
    @test log(Taylor(1,[1,1]))==Taylor(1,[0,1])#logaritmo
end